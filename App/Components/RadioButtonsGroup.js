  ...
  render() {
    ...
    return (
      <TouchableOpacity
        ...
        <View style={{flexDirection: 'row', borderWidth: 1, borderColor: '#86cacb', borderRadius: 10, padding: 10, alignItems: 'center', justifyContent: 'center'}}>
          <View
            style={[
              styles.border,
              {
          ...
          <Text style={[{ alignSelf: 'center', fontSize: data.selected ? 14 : 15, color: 'black', fontWeight: data.selected ? 'bold' : 'normal'}, margin]}>{data.label}</Text>
        </View>
      </TouchableOpacity>
    );
  }
